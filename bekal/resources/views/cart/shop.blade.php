...
@section('content')
    ...
    <div class="container">
        ...
        @foreach ($paket->chunk(4) as $items)
            <div class="row">
                @foreach ($items as $product)
                    <div class="col-md-3">
                        <div class="thumbnail">
                            <div class="caption text-center">
                                <a href="{{ url('shop', [$product->nama_paket]) }}"><img src="{{ asset('fotoupload/' . $product->foto_paket) }}" alt="product" class="img-responsive"></a>
                                <a href="{{ url('shop', [$product->nama_paket]) }}"><h3>{{ $product->nama_paket }}</h3>
                                <p>{{ $product->harga_paket }}</p>
                                </a>
                            </div> <!-- end caption -->
                        </div> <!-- end thumbnail -->
                    </div> <!-- end col-md-3 -->
